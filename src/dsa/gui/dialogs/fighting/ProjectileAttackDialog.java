...
public final class ProjectileAttackDialog extends BGDialog {
  ...
  private FarRangedFightParams params;
  
  private int additionalModifier = 0;
   ...
   * 
   */
  public ProjectileAttackDialog(JFrame owner, int at, Weapon weapon, FarRangedFightParams params) {
    super(owner, true);
    this.params = params;
    baseAT = at;
    this.weapon = weapon;
    initialize();
    getModifierButton().setEnabled(false);
    initValues();
    setLocationRelativeTo(owner);
    calcSums();
  }
  
  public ProjectileAttackDialog(JDialog owner, int weaponNr, 
      Fighter attacker, int additionalMod) {
    super(owner, true);
    this.params = attacker.getFarRangedFightParams();
    fighter = attacker;
    Optional<Integer> atValue = attacker.getAT(weaponNr);
    baseAT = atValue.hasValue() ? atValue.getValue() : 7;
    String weaponName = attacker.getFightingWeapons().get(weaponNr);
    weapon = Weapons.getInstance().getWeapon(weaponName);
    this.weaponNr = weaponNr;
    initialize();
    initValues();
    setLocationRelativeTo(owner);
    additionalModifier = additionalMod;
    calcSums();
  }
  ...
  private void initValues() {
    distanceSpinner.setValue(params.getDistance());
    sizeBox.setSelectedIndex(params.getSize());
    movementBox.setSelectedIndex(params.getMovement());
    sightBox.setSelectedIndex(params.getSight());
    windBox.setSelectedIndex(params.getWind());
    fightSpinner.setValue(params.getInfights());
    shootBox.setSelectedIndex(params.getMode());
  }
  ...
  private void saveValues(FarRangedFightParams params1) {
    ...
    params1.setWind(windBox.getSelectedIndex());
    params1.setInfights(((Number)fightSpinner.getValue()).intValue());
    params1.setMode(shootBox.getSelectedIndex());
  }
  ...
  private JPanel getJContentPane() {
    if (jContentPane == null) {
      ...
      jContentPane.add(getModifierButton(), null);
    }
    return jContentPane;
  }
  ...
  private JButton modifierButton = null;
  ...
  public int getModifier() {
    return modifier - additionalModifier;
  }
  ...
  public static int calcModifier(FarRangedFightParams p, Weapon w) {
    int modifier = 0;
    switch (w.getDistanceCategory(p.getDistance())) {
    case 0:
      modifier -= 4;
      break;
    case 1:
    ...
    modifier += p.getSight();
    modifier += p.getWind();
    modifier += 3 * p.getInfights();
    return modifier;
  }
  
  public static int calcAtValue(FarRangedFightParams p, int baseAT) {
    int atValue = baseAT;
    switch (p.getMode()) {
    case 0:
      atValue = baseAT / 2;
  ...
  private void calcSums() {
    FarRangedFightParams params1 = new FarRangedFightParams();
    saveValues(params1);
    modifier = calcModifier(params1, weapon); 
    modifier += additionalModifier;
    atValue = calcAtValue(params1, baseAT);
  ...
  private JButton getOKButton() {
    if (okButton == null) {
      ...
      okButton.setBounds(new Rectangle(10, 270, 101, 21));
      okButton.setText("OK");
  ...
  private JButton getCancelButton() {
    if (jButton == null) {
      jButton = new JButton();
      jButton.setBounds(new Rectangle(120, 270, 101, 21));
  ...
  /**
   * This method initializes modifierButton	
   * 	
   * @return javax.swing.JButton	
   */
  private JButton getModifierButton() {
    if (modifierButton == null) {
      ...
      modifierButton.setBounds(new Rectangle(230, 270, 101, 21));
      modifierButton.setToolTipText("Modifikatoren");
      modifierButton.setText("Mod. ...");
      modifierButton.addActionListener(new java.awt.event.ActionListener() {
        public void actionPerformed(java.awt.event.ActionEvent e) {
          AtPaModDialog subDialog = new AtPaModDialog(ProjectileAttackDialog.this, fighter, weaponNr, additionalModifier, true);
          subDialog.setVisible(true);
          additionalModifier = subDialog.getExtraMod();
          calcSums();
        }
      });
    }
