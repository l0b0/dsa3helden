...
public class CharacterPrinter {
  ...
  private static class OriginComparator implements Comparator<Spell> {
    ...
    public int compare(Spell o1, Spell o2) {
      int r = o1.getOrigin().compareTo(o2.getOrigin());
      return dir == 0 ? 0 : (dir == 1 ? r : -r);
    }
  }

  private static class CategoryComparator implements Comparator<Spell> {
    public CategoryComparator(int d) {
      dir = d;
    }
    ...
    public int compare(Spell o1, Spell o2) {
      int r = o1.getCategory().compareTo(o2.getCategory());
      return dir == 0 ? 0 : (dir == 1 ? r : -r);
    }
  }

  private static class ValueComparator implements Comparator<Spell> {
    public ValueComparator(int d, Hero h) {
      hero = h;
      dir = d;
    }

    public int compare(Spell o1, Spell o2) {
      int v1 = hero.getDefaultTalentValue(o1.getName());
      int v2 = hero.getDefaultTalentValue(o2.getName());
  ...
  private static class SpellSorter {
    ...
    private ArrayList<Comparator<Spell>> comparators;
  ...
  public void printCharacter(Hero character, File template, File output,
    ...
    dsa.gui.frames.SpellFrame spellFrame = (dsa.gui.frames.SpellFrame) (dsa.gui.frames.FrameManagement
    ...
    for (String parade : character.getShields()) {
      dsa.model.data.Shield shield = dsa.model.data.Shields.getInstance().getShield(parade);
      ...
      table.addItem("SBF" + paradeNr, "" + shield.getBf());
    ...
    for (String name : character.getWeapons()) {
      ...
      if (!weapon.isProjectileWeapon()) { // Nahkampfwaffe
        ...
        table.addItem("wb" + weaponNr, "" + weapon.getBF());
        table.addItem("Wb", "" + weapon.getBF(), true);
        ...
        for (int i = 0; i < Weapon.Distance.values().length; ++i) {
          ...
          fr += weapon.getDistancePaces(Weapon.Distance.values()[i]);
          int m = weapon.getDistanceCategoryTPMod(Weapon.Distance.values()[i]);
          if (m > 0) fm += "+";
          fm += m;
        }
        table.addItem("fr" + fkWeaponNr, fr);
        table.addItem("fm" + fkWeaponNr, fm);
  ...
  private static CharacterPrinter instance;
}
